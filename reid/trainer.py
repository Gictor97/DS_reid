...
from .loss import TripletLoss, CrossEntropyLabelSmooth, TripletLossXBM, DivLoss, BridgeFeatLoss, BridgeProbLoss,domain_regluarization_loss,DDC_loss
...
class idm_trainer(Base_trainer):
    def __init__(self,model,num_classes,mu1=1.0,mu2=1.0,mu3=1.0,xbm = None,margin=None):
        super(Base_trainer,self).__init__()
        self.model = model
        self.mu1 = mu1
        self.mu2 = mu2
    ...
    def train(self,epoch,stage,data_source_loader,data_target_loader,optim,s_class,t_class,iters,gamma,print_freq=50):
       ...
       ##calculate
            ...
            if self.xbm:
                ...
                loss = (1.0 - self.mu1) * loss_ce + loss_tri  + self.mu1 * loss_bridge_pro + \
                       +self.mu2 * loss_brifea + self.mu3 * loss_div + gamma *loss_re
            ...
            loss_dives.update(mmdloss.item())
