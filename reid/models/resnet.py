...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        resnet = ResNet.__factory[depth](pretrained=pretrained)  ###加载模型,会将参数模型一起下载
    ...
    def forward(self, x, output_prob=False):
        ...
        if self.has_embedding:
            #False，True的话就是先经过一个channel变化，将pool之后的特征层进行一个BNneck
...
if __name__=='__main__':
    print('a')
    model = resnet50(num_classes = 13682)
    data = torch.Tensor(64,3,256,128)
    a,b  = model(data)
    set_trace()