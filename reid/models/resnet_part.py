...
class ResNetPart(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, num_parts=3, num_classes=0):
        ...
        self.gap = nn.AdaptiveAvgPool2d(1)   ## global
        self.rap = nn.AdaptiveAvgPool2d((self.num_parts, 1)) ##part

        # global feature classifiers
        self.bnneck = nn.BatchNorm1d(2048)
        init.constant_(self.bnneck.weight, 1)
        init.constant_(self.bnneck.bias, 0)  ###这样就不用bias参数了
    ...
    def forward(self, x):
        ...
        if self.training is False:  #非训练模式
            f_g = F.normalize(f_g)
            return f_g
